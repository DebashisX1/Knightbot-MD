    ...
    try {
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.kick'):
                ...
                break;
            case userMessage.startsWith('.fquote'):
                {
                    const parts = userMessage.slice(7).trim();
                    if (!parts) {
                        await sock.sendMessage(chatId, { text: 'Usage: .fquote number,text1,text2' }, { quoted: message });
                        break;
                    }
                    
                    const commaParts = parts.split(',');
                    if (commaParts.length !== 3) {
                        await sock.sendMessage(chatId, { text: 'Usage: .fquote number,text1,text2' }, { quoted: message });
                        break;
                    }
                    
                    const [phoneNumber, text1, text2] = commaParts.map(part => part.trim());
                    
                    // Create a fake quoted message object
            ...
            case userMessage === '.unmute':
                ...
                break;
            case userMessage === '.button':
                {
                    console.log('Button command triggered');
                    try {
                        await sock.sendMessage(chatId, {
                            ...
                            headerType: 1,
                            footerText: "Demo Interactive Message"
                        }, { quoted: message });
                        
                        console.log('Button message sent successfully');
                    } catch (error) {
                        console.error('Error in button command:', error);
                        await sock.sendMessage(chatId, { text: 'Error sending button message' }, { quoted: message });
                    }
                }
                break;
